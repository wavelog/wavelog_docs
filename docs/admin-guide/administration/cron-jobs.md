...
!!! tip
    By default, the cronmanager uses the url configured as `$config['base_url']` within config.php when making internal calls. If this url is not reachable from the host where wavelog is installed (this could happen especially when using a [docker setup](../../getting-started/installation/docker.md)) the cronmanager will fail. For such cases it is possible to configure a url which should be used for local, internal calls using `$config['local_url']` within the config.php. For a setup within a docker container this snippet would do the job:
